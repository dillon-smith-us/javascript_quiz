...
response.setAttribute("style", "display: block; color: yellowgreen;");
...
function reset() {
    ...
    root.children[0].textContent = "JAVASCRIPT QUIZ";
...
function answerCheck(element) {
    if ((questionNumber - 1) < questions.length) {
        if (correct[questionNumber - 1][parseInt(element.id) - 1]) {
            response.textContent = "Right! "
            response.setAttribute("style", "display: block; text-align: left: margin-left: 20%; margin-top: 30px; padding-top: 8px; border-top: 2px solid green;");
            prompty = time + 1;
        } else {
            time -= 10;
